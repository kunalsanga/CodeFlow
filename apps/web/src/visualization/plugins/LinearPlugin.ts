...
import { ISemanticIRv1 } from '@codeflow/types';

export class LinearPlugin {
  public static buildScene(semanticIR: ISemanticIRv1 | any): SceneGraph {
    const currentStep = semanticIR?.metadata?.currentStep || 0;
    const scene = new SceneGraph({
      title: 'Singly Linked List',
      subtitle: `Step ${currentStep + 1} of ${semanticIR?.metadata?.totalSteps || 10}`,
    });
